# Running RCO Checkout Sign In and Samples Mobile Test 

## As a New User I would like to Register from Checkout Sign In and complete checkout for Mobile if purchase Sample + Normal SKU
* APACRCO Initialize Helix
* APACACCCHK Initialize Helix
* APACACCCHK Configuring the prerequisites like set cookies, set test Order flag, revision tag
* APACRCO Verify that the user is able to add products to the cart
* APACACCCHK Verify that the user is able to view and update the cart
* APACRCO Verify that user is able to add complimentary/Intelligent samples in Cart Page
* APACACCCHK Verify that the user is able to proceed to "Newuser" Sign in successfully
* APACACCCHK Verify that the user is able to enter the "Newuser" shipping details successfully
* APACRCO Verify that user is able to place orders with COD Radio button
* APACRCO Verify Order is placed and Order ID displayed