# Running Guest User Different Delivery MOB Test

##  Verify Guest User Is Able To Complete CC Checkout With NEXTDAY Delivery types

* UKACCCHK Initialize Helix
* UKPAYMENT Initialize Helix
* UKACCCHK Configure Pre-Requisites
* UKPAYMENT Verify that the user able to add products into cart
* UKPAYMENT Verify that the user is able to select delivery type "Next Day" from dropdown
* UKPAYMENT Verify that the user able to signin as a guest user
* UKACCCHK Verify that the user is able to enter the user shipping details successfully "GU"
* UKACCCHK Verify that the user is able to select the payment method successfully "GU"
* UKACCCHK Verify that the user is able to place the order successfully