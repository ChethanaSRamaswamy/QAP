# Running Return User PC Test

## As a Return User I would like to Add a product and complete checkout.
* APACACCCHK Initialize Helix
* GA Initialize Helix
* GA STORELOCATOR Configuring the prerequisites like set cookies, revision tag
* GA Intercept for Analytics
* APACACCCHK Verify that the user is able to add products to the cart successfully from prodcat
* GA Analytics validation
* APACACCCHK Verify that the user is able to view and update the cart
* GA Analytics validation
* APACACCCHK Verify that the user is able to proceed to "Returnuser" Sign in successfully
* GA Analytics validation
* APACACCCHK Verify that the user is able to enter the "Returnuser" shipping details successfully
* GA Analytics validation
* APACACCCHK Verify that the user is able to select the payment method successfully
* GA Analytics validation
* APACACCCHK Verify that the user is able to place the order successfully
* GA Analytics validation
* GA Analytics validation result